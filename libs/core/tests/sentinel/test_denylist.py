...
class TestEnvReadingDenylist:
    """Tests for environment variable exfiltration denylist patterns."""

    def setup_method(self) -> None:
        self.policy = DenylistPolicy()

    def test_printenv_denied(self) -> None:
        req = ToolCallRequest(tool="bash", command="printenv")
        result = self.policy.evaluate(req)
        assert result.decision == PolicyDecision.DENY

    def test_printenv_with_arg_denied(self) -> None:
        req = ToolCallRequest(tool="bash", command="printenv HOME")
        result = self.policy.evaluate(req)
        assert result.decision == PolicyDecision.DENY

    def test_bare_env_denied(self) -> None:
        req = ToolCallRequest(tool="bash", command="env")
        result = self.policy.evaluate(req)
        assert result.decision == PolicyDecision.DENY

    def test_env_pipe_denied(self) -> None:
        req = ToolCallRequest(tool="bash", command="env | grep KEY")
        result = self.policy.evaluate(req)
        assert result.decision == PolicyDecision.DENY

    def test_env_redirect_denied(self) -> None:
        req = ToolCallRequest(tool="bash", command="env > /tmp/vars.txt")
        result = self.policy.evaluate(req)
        assert result.decision == PolicyDecision.DENY

    def test_env_with_var_assignment_allowed(self) -> None:
        req = ToolCallRequest(tool="bash", command="env LANG=C python script.py")
        result = self.policy.evaluate(req)
        assert result.decision == PolicyDecision.ABSTAIN

    def test_os_environ_denied(self) -> None:
        req = ToolCallRequest(tool="bash", command="python -c 'import os; print(os.environ)'")
        result = self.policy.evaluate(req)
        assert result.decision == PolicyDecision.DENY

    def test_process_env_denied(self) -> None:
        req = ToolCallRequest(tool="bash", command="node -e 'console.log(process.env)'")
        result = self.policy.evaluate(req)
        assert result.decision == PolicyDecision.DENY

    def test_echo_api_key_var_denied(self) -> None:
        req = ToolCallRequest(tool="bash", command="echo $ANTHROPIC_API_KEY")
        result = self.policy.evaluate(req)
        assert result.decision == PolicyDecision.DENY

    def test_echo_token_var_denied(self) -> None:
        req = ToolCallRequest(tool="bash", command="echo $GITHUB_TOKEN")
        result = self.policy.evaluate(req)
        assert result.decision == PolicyDecision.DENY

    def test_echo_secret_var_denied(self) -> None:
        req = ToolCallRequest(tool="bash", command="echo ${AZURE_CLIENT_SECRET}")
        result = self.policy.evaluate(req)
        assert result.decision == PolicyDecision.DENY

    def test_echo_password_denied(self) -> None:
        req = ToolCallRequest(tool="bash", command="echo $DB_PASSWORD")
        result = self.policy.evaluate(req)
        assert result.decision == PolicyDecision.DENY

    def test_windows_percent_var_denied(self) -> None:
        req = ToolCallRequest(tool="bash", command="echo %ANTHROPIC_API_KEY%")
        result = self.policy.evaluate(req)
        assert result.decision == PolicyDecision.DENY

    def test_echo_safe_var_allowed(self) -> None:
        req = ToolCallRequest(tool="bash", command="echo $HOME")
        result = self.policy.evaluate(req)
        assert result.decision == PolicyDecision.ABSTAIN

    def test_echo_safe_string_allowed(self) -> None:
        req = ToolCallRequest(tool="bash", command="echo hello world")
        result = self.policy.evaluate(req)
        assert result.decision == PolicyDecision.ABSTAIN

    def test_credential_var_denied(self) -> None:
        req = ToolCallRequest(tool="bash", command="cat $AWS_CREDENTIAL")
        result = self.policy.evaluate(req)
        assert result.decision == PolicyDecision.DENY
