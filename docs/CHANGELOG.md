...
- **Agent definitions** -- `openclaw/config/agents.json` defines the four production agents (Vizier, Pasha, Worker, Quality Gate) with their models, workspace paths, MCP server bindings, and parent/child relationships. The routing table directs all unaddressed messages to the Vizier agent.
